<!-- ========== CONTACTS SECTION ========== -->
<section id="contact" name="contact"></section>
<div id="f">
  <div class="container">
    <div class="row">
      <div class="col-lg-6">
        <h3><b>Contact Us</b></h3>
        <p>
          <a href="index.html#"><i class="icon-envelope"></i></a>
        </p>
      </div>

      <div class="col-lg-6">
        <h3><b>Talk about us:</b></h3>
        <p>
          <a href="index.html#"><i class="icon-facebook"></i></a>
          <a href="index.html#"><i class="icon-twitter"></i></a>
          <a href="index.html#"><i class="icon-google-plus"></i></a>
        </p>
        <br>
      </div>
    </div>
  </div>
</div>

<!-- Bootstrap core JavaScript
================================================== -->
<!-- Placed at the end of the document so the pages load faster -->
<script src="{$BASE_URL}javascript/jquery.js"></script>
<script src="{$BASE_URL}javascript/classie.js"></script>
<script src="{$BASE_URL}javascript/bootstrap.min.js"></script>
<script src="{$BASE_URL}javascript/smoothscroll.js"></script>
<script src="{$BASE_URL}javascript/main.js"></script>
<script src="{$BASE_URL}javascript/jquery.easing.min.js"></script>
<script src="{$BASE_URL}javascript/scrolling-nav.js"></script>
</body>
</html>
